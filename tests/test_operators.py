...
@pytest.mark.parametrize("status_checker, expect_passes",
                         [(lambda x : True, True), 
...
def test_check_run_result_operator(mocker, status_checker, expect_passes):
    ...
    mock_client.get_table_information.return_value = {'id': "foo_id"}
    mock_client.get_check_intervals.return_value = [{"latest_run_checks_job_id": "anomalo_job_id"}]
    run_result_mock = mocker.MagicMock()
    mock_client.get_run_result.return_value = run_result_mock

    check_run = AnomaloCheckRunResultOperator(table_name="foo",
    ...
    mock_client.get_table_information.assert_called_with(table_name="foo")
    mock_client.get_check_intervals.assert_called_with(table_id="foo_id", start="2023-02-01", end=None)
...
def create_mock_run_result(type_to_success: List[Tuple[str, bool]]) -> Mapping:
    ...
    for check_type, success in type_to_success:
        runs.append({"run_config": {"_metadata": {"check_type": check_type}}, "results": {"success": success}})
    return {"check_runs": runs}


@pytest.mark.parametrize("results, must_pass, expect_passes",
                         [(create_mock_run_result([("rule", True)]), ["rule"], True),
                          ...
                          (create_mock_run_result([("rule", True), ("metric", False)]), ["rule", "metric"], False)])
def test_check_pass_fail_operator(mocker, results, must_pass, expect_passes):
    print(results)

    check_pass = AnomaloPassFailOperator(table_name="foo",
                                         run_date=date(2023, 2, 1),
                                         must_pass=must_pass,
                                         task_id="bar")
